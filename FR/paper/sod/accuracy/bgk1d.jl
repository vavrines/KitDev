...
begin
    cd(@__DIR__)
    @load "kn2_ref.jld2" x_ref u_ref

    w = zeros(length(x_ref), 3)
    prim_ref = zeros(length(x_ref), 4)
    for i = 1:size(u_ref, 1)
        ...
        for j = 1:size(u_ref, 3)
            ...
            w[idx, :] .= u_ref[i, 1:3, j]
            prim_ref[idx, 1:3] .= conserve_prim(w[idx, :], 5 / 3)
            prim_ref[idx, 4] = 0.5 * prim_ref[idx, 1] / prim_ref[idx, 3]
        end
    end
end
